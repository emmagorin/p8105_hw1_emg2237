...
Use the code below to download the a package containing the penguins dataset _(done)_
...
_The "penguins" dataset includes information on `r ncol(palmerpenguins::penguins)` variables (ie, it has `r ncol(palmerpenguins::penguins)` columns) for `r nrow(palmerpenguins::penguins)` penguins (ie, it has `r nrow(palmerpenguins::penguins)` rows). Some key variables include species, which includes `r levels(palmerpenguins::penguins$species)`, island, which includes `r levels(palmerpenguins::penguins$island)`, and basic measurements of the penguins such as flipper length, which uses the variable name flipper_length_mm and ranges from `r min(palmerpenguins::penguins$flipper_length_mm, na.rm = TRUE)`mm to `r max(palmerpenguins::penguins$flipper_length_mm, na.rm = TRUE)`mm. The mean flipper length for penguins for whom flipper length is available in this dataset is `r mean(palmerpenguins::penguins$flipper_length_mm, na.rm = TRUE)`mm._
